...
# Function to create float input with steep increment logic
def float_input(label, key, placeholder="", Steep=10000, required=False):
    if required:
        label += " *"
    # Get the current value from session state or use a default value
    if key not in st.session_state:
        st.session_state[key] = 300000  # Default value for home price
    
    # Display the current value
    val = st.session_state[key]

    # Show text input for the field
    new_value = st.sidebar.text_input(label, key=key, placeholder=placeholder, value=f"{val:,.0f}")
    
    try:
        # Convert text input to float if it's valid
        val = float(new_value)
    except:
        pass  # If it's not a valid float, keep the previous value
    
    # Show button to increase the value by Steep
...
hoa = float_input("HOA $", "hoa", "e.g. 250", Steep=100, required=True)
property_tax_rate = float_input("Property Tax %", "tax", "e.g. 1.2", Steep=0.1, required=True)
insurance_rate = float_input("Insurance %", "insurance", "e.g. 0.5", Steep=0.05, required=True)
pmi_rate = float_input("PMI %", "pmi", "e.g. 0.5", Steep=0.05, required=True)
...
cash_available = float_input("Cash Available $", "cash", "e.g. 80000", Steep=5000, required=True)
monthly_liability = float_input("Monthly Liability $", "liability", "e.g. 500", Steep=100, required=True)
...
max_monthly_expense = float_input("Max Monthly Expense $", "max_exp", "e.g. 2200", Steep=500)

...
def float_input(label, key, placeholder="", step=1.0, required=False, decimals=2):
    if required:
        label += " *"
    fmt = f"%.{decimals}f"
...
hoa = float_input("HOA $", "hoa", "e.g. 250", step=10.0, required=True, decimals=0)
property_tax_rate = float_input("Property Tax %", "tax", "e.g. 1.2", step=0.1, required=True, decimals=2)
...
annual_income = float_input("Annual Income $", "income", "e.g. 85000", step=10000.0, required=True, decimals=0)
max_dti = float_input("Max DTI %", "dti", "e.g. 36", step=1, required=True, decimals=2)


# Optional inputs (no *)
min_down_pct = float_input("Min Down Payment %", "min_dp", "e.g. 5", step=1.0, decimals=2)
max_down_pct = float_input("Max Down Payment %", "max_dp", "e.g. 20", step=1.0, decimals=2)
max_monthly_expense = float_input("Max Monthly Expense $", "max_exp", "e.g. 2200", step=100.0, decimals=0)

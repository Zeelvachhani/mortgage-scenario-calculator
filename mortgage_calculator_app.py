...
def inline_number_input(label, key, **kwargs):
    ...
    return col1.text(label), col2.number_input(label="", key=key, **kwargs)[1]

def inline_text_input(label, key):
    col1, col2 = st.columns([1, 3], gap="small")
    return col1.text(label), col2.text_input(label="", key=key)[1]

# --- LEFT: Inputs ---
with left_col:
    st.subheader("\U0001F4DD Input Parameters")

    _, home_price = inline_number_input("Home Price $:", "home_price", min_value=0.0, step=1000.0)
    _, hoa = inline_number_input("HOA $:", "hoa", min_value=0.0, step=10.0)
    _, property_tax_rate = inline_number_input("Property Tax %:", "tax", min_value=0.0, step=0.1)
    property_tax_rate /= 100
    _, insurance_rate = inline_number_input("Insurance %:", "insurance", min_value=0.0, step=0.1)
    insurance_rate /= 100
    _, pmi_rate = inline_number_input("PMI %:", "pmi", min_value=0.0, step=0.1)
    pmi_rate /= 100

    _, cash_available = inline_number_input("Cash Available $:", "cash", min_value=0.0, step=1000.0)
    _, min_down_str = inline_text_input("Min Down Payment % (optional):", "min_dp")
    _, max_down_str = inline_text_input("Max Down Payment % (optional):", "max_dp")
    _, interest_rate_base = inline_number_input("Interest Rate %:", "rate", min_value=0.0, step=0.01)
    interest_rate_base /= 100
    _, loan_term = inline_number_input("Loan Term (Years):", "term", min_value=1, step=1, value=30)
    loan_term = int(loan_term)

    _, monthly_liability = inline_number_input("Monthly Liability $:", "liability", min_value=0.0, step=100.0)
    _, annual_income = inline_number_input("Annual Income $:", "income", min_value=0.0, step=1000.0)
    _, max_dti = inline_number_input("Max DTI %:", "dti", min_value=0.0, max_value=100.0, step=1.0)
    max_dti /= 100
    _, max_monthly_expense_str = inline_text_input("Max Monthly Expense $ (optional):", "max_exp")
